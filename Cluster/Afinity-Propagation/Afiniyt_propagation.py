...
if __name__ == "__main__":
    ...
    data, y = ds.make_blobs(N, n_features=2, centers=centers, cluster_std=[0.5, 0.25, 0.7, 0.5], random_state=66)
    ...
    for i, mul in enumerate(np.linspace(1, 4, 9)):
        ...
        print(('p = %.1f' % mul), p, 'the number of clusters is:', n_clusters)
        y_hat = af.labels_

        plt.subplot(3, 3, i+1)
        plt.title('Preference: %.2f, the number of clusters:%d' % (p, n_clusters))
        clrs = []
        ...
        plt.grid(True)
    plt.tight_layout()
    plt.suptitle('AP cluster', fontsize=20)
