...
if __name__ == "__main__":
    ...
    plt.suptitle('DBSCAN cluster', fontsize=20)

    for i in range(6):
        eps, min_samples = params[i]
        model = hdbscan(eps=eps, min_samples=min_samples)
        model.fit(data)
        y_hat = model.labels_
        ...
        plt.grid(True)
        plt.title('epsilon:%.1f,m:%d,cluster num:%d' % (eps, min_samples, n_clusters), fontsize=16)
    plt.tight_layout()
