...
def mle():  # 0Begin/1Middle/2End/3Single
    pi = [0] * 4   # npi[i]：i状态的个数。因为是要计算状态的初始概率，所以只计算句子第一个词（即跟在，；。）之后的词）
    a = [[0] * 4 for x in range(4)]     # na[i][j]：从i状态到j状态的转移个数
    b = [[0]* 65536 for x in range(4)]  # nb[i][o]：从i状态到o字符的个数
    f = open("data/pku_training.utf8", encoding="utf-8")
    data = f.read()[3:]
    f.close()
    tokens = data.split('  ')
    # # 增加英文词训练集
...
def save_parameter(pi, A, B):
    f_pi = open("data/pi.txt", "w", encoding='utf-8')
    list_write(f_pi, pi)
    f_pi.close()
    f_A = open("data/A.txt", "w")
    for a in A:
        list_write(f_A, a)
    f_A.close()
    f_B = open("data/B.txt", "w")
    for b in B:
        list_write(f_B, b)
    f_B.close()
