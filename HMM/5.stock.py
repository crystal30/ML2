...
if __name__ == "__main__":
    ...
    x = np.loadtxt('./data/SH600000.txt', encoding='utf-8', delimiter='\t', skiprows=2, usecols=(4, 5, 6, 2, 3))
    close_price = x[:, 0]
    volumn = x[:, 1]
    amount = x[:, 2]
    ...
    plt.grid(True)
    plt.title('quote change')  # 涨跌幅
    plt.subplot(422)
    plt.plot(t, volumn, 'g-')
    plt.grid(True)
    plt.title('volumn')  # 交易量
    ...
    for i, clr in enumerate(clrs):
        ...
        plt.grid(True)
    plt.suptitle('SH600000：GaussianHMM decompose hidden variables', fontsize=18)
    plt.tight_layout()
